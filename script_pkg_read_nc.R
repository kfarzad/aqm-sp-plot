in_nc_out_rast <- function(input_path,
    ...
    if (debug) {
        print(paste0("outp_vars: ", outp_vars))
    }
    if (outp_vars) {
...
if (FALSE) {
    ...
    pm25_raster <- in_nc_out_rast(input_path = thepath, input_nc_name = "test.nc", silent = FALSE)
    in_nc_out_rast(input_path = thepath, input_nc_name = "test.nc", outp_vars = TRUE, debug = TRUE)
    in_nc_out_rast(input_path = thepath, input_nc_name = "test.nc", outp_info = TRUE)
}
