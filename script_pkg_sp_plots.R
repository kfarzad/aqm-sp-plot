...
world_longlat <-
    sf::as_Spatial(
        sf::st_boundary(
            sf::st_as_sf(
                maps::map(
                    database = "world", regions = ".", fill = TRUE, plot = FALSE
                ),
                crs = 4326
            )
        )
...
extract_zone <- function(
    ...
    dotfiles_path) {
    source(paste0(dotfiles_path, "/", "script_pkg_regions.R"))
...
add_overlay <- function(
    ...
    if (debug) print(head(df_long_lat_val))
    ...
    if (plot_proj_mode == "longlat") {
        ...
        bin_range <- conLimRange / length(col)
        ...
        ## Add plot as a layer
        raster::plot(inp, bg = col, pch = 21, cex = cex, add = TRUE)
    }
...
spatial_plot <- function(raster_2_plot,
    ...
    suppressWarnings({
        ...
        if (missing(dotfiles_path)) {
            ...
            for (dotfiles_path in dotfiles_paths) {
                if (dir.exists(dotfiles_path)) {
                    dotfiles_mcp_path <- paste0(dotfiles_path, "models/cmaq/post/")
                    break
            ...
            dotfiles_mcp_path <- dotfiles_path
        }
        ...
        if (!exists("file_crs") && !is.na(terra::crs(raster_2_plot)) && plot_proj_mode == "lcc") {
            ...
            source(paste0(dotfiles_mcp_path, "/", "script_pkg_sp_plots.R"))
        }
        ...
        if (debug) {
            print(dotfiles_mcp_path)
        }
        source(paste0(dotfiles_mcp_path, "/", "script_pkg_title.R"))

        make_break_func <- function(conLim = conLim, col = col,
            ...
            if (!break_points) {
                ...
                if (!break_points_mid_white) {
                    break_points <- c(seq(
                        from = conLim[1],
                        to = conLim[2],
                        by = (conLim[2] - conLim[1]) / length(col)
                    ))
                } else {
                    break_points <- c(seq(
                        from = conLim[1] - ((conLim[2] - conLim[1]) / (length(col) - 1)) / 2,
                        to = conLim[2] + ((conLim[2] - conLim[1]) / (length(col) - 1)) / 2,
                        by = (conLim[2] - conLim[1]) / (length(col) - 1)
                    ))
                }
            }
            return(break_points)
            ...
            for (format in output_formats) {
                ...
                if (zone_map) {
                    if (zone_def == "") {
                        ...
                        return(extract_zone(
                            def = zone_def,
                            dotfiles_path = dotfiles_mcp_path
                        ))
                    }
                ...
                if (custom_legend) {
                    ...
                    legend(
                        ...
                        fill = col, cex = custom_legend_cex
                    )
                }
                ## Close Saving File
                dev.off()
                ...
                if (zone_map) {
                    ...
                    for (zone in 1:extract_zone(
                        ...
                        temp_zone <- extract_zone(zone, zone_def,
                            ...
                            silent = !debug, dotfiles_path = dotfiles_mcp_path
                        )
                        raster::plot(temp_zone, add = TRUE, lwd = zone_map_lwd)
                        #         },
                        #         silent = !debug
...
if (FALSE) {
    thepath <- R.utils::getParent(this.path::this.path())
    source(paste0(thepath, "/script_pkg_read_nc.R"))
    pm25 <- in_nc_out_rast(input_path = thepath, input_nc_name = "test.nc")
    write.csv(data.frame(A_LAT = c(42.3398), VALUESSSSSS = c(15), X_LON = c(-71.0892)),
        file = paste0(thepath, "/test.csv"),
        row.names = FALSE
    )
}
if (FALSE) {
    thepath <- R.utils::getParent(this.path::this.path())
    pm25 <- raster::raster(paste0(thepath, "/", "test.gri"))
    source(paste0(thepath, "/script_pkg_sp_plots.R"))
    source(paste0(thepath, "/script_pkg_vars_spec.R"))
    variable <- "pm25"
    spatial_plot(
        raster_2_plot = pm25,
        ...
        df_long_lat_val = read.csv(paste0(thepath, "/test.csv"))[, c(3, 1, 2)],
        # unit = "ugm-3",
        # specie = "PM2.5",
        # conLim = c(0, 15),
        plot_proj_mode = "lcc",
        overlay_plot_switch = TRUE,
        l_legend = TRUE,
        col = myColorSimple(200),
        title = "PM2.5 Concetration",
        output_formats = c("png", "pdf"),
        quick_stats = TRUE,
        plot_mask_switch = FALSE,
        output_name = paste0("test_sp_plot"),
        output_path = paste0(thepath),
        zone_map = TRUE,
        zone_def = "cr",
        zone_map_lwd = 4,
        dotfiles_path = thepath,
        debug = TRUE
    )
}
